...
**补充硬规：** 同一页面里若已经存在主 CTA（如「开始聊天」），则摘要卡、提示条、Hero 卡、空态卡里不得再新增同名按钮或等价跳转入口；它们只能做信息展示或辅助说明。
...
- [ ] 同一页面是否已存在主 CTA？有 → 不要在 Hero / 摘要 / 空态再复制一份
...
| Companion 首页「pulse」 vs 「开始聊天」 | 只保留一个主聊天 CTA | 其他等价聊天入口 |
...
### 2.2 外部 Flutter 参考库（精选使用）

参考索引：[`Solido/awesome-flutter`](https://github.com/Solido/awesome-flutter)。它是发现资料和组件的目录，不是依赖清单；禁止因为条目有 Star 就直接引入项目。

**优先参考的方向：**

- 官方 Flutter Gallery / Samples：先看 Flutter 原生组件、Material/Cupertino 适配和可访问性实现。
- Timeline / Timeline Tile：可用于 `TA 的日常` 关系时间线；先用现有 `CompanionDailyItem` 和本地组件，只有交互复杂度确实超过自建实现时才引包。
- `super_sliver_list`：当日常流、Feed 或聊天历史出现大数据量和可变高度性能问题时评估，不为当前 16 条摘要列表预先引入。
- `flutter_secure_storage`、`flutter_local_notifications`：分别用于用户 Provider 密钥和主动陪伴通知；密钥不进普通偏好存储，通知必须有权限、免打扰和关闭路径。
- `flutter_blurhash`、`cached_network_image`：图片密集页面可评估占位和缓存；必须验证空图、失败图、内存释放和平台行为。
- `flutter_webrtc` / 音频类组件：只有 Companion 语音通话需求进入当前切片时才评估，不能先为“未来沉浸感”引入原生依赖。

**引入前四问：**

1. 仓库现有 Moe widget、Service、Token 或平台能力能否解决？能 → 不引包。
2. 组件是否支持当前目标平台、无障碍语义、键盘/触摸和窄屏布局？不支持 → 不进入主路径。
3. 是否有明确的数据量、交互或平台能力问题可以被它解决？没有 → 只记录参考，不改依赖。
4. 许可证、维护活跃度、包体积、原生权限和回滚方案是否清楚？不清楚 → 先做小型 spike，不进正式代码。

**Companion 映射：**

- 关系时间线 → 先保持单一叙事和自建 `DailyTile`，必要时再评估 Timeline。
- 主动消息 → 先复用通知中心和现有 WS，再评估本地通知插件；不另建第二套推送状态。
- 用户模型密钥 → 继续使用 `flutter_secure_storage`，Web 端明确提示存储边界。
- 图片/自拍 → 先补失败、缓存和无图状态，再评估图片组件；不为视觉效果堆叠滤镜/动画包。
