...
- **自动切换目录**：创建 worktree 后**自动切换到新目录**进行开发
...
# 在独立目录创建 worktree
worktree_dir="../moe_social_<功能名>"
git worktree add ${worktree_dir} feature/ai-${current_branch}-<功能名>

# 【关键步骤】切换到 worktree 目录
cd ${worktree_dir}
...
### 2. 开发阶段（必须在 worktree 目录中进行）

**⚠️ 重要：所有开发工作必须在 worktree 目录中进行，不要在原目录修改！**
...
## 使用示例
- 当前分支: `main`，"帮我开发一个新功能：用户头像上传" → 创建 `feature/ai-main-user-avatar-upload` 和 `../moe_social_user-avatar-upload`，**自动切换到新目录**
- 当前分支: `develop`，"修复登录页面的 bug" → 创建 `fix/ai-develop-login-page-bug` 和 `../moe_social_fix_login-page-bug`，**自动切换到新目录**
...
- **创建 worktree 后必须立即切换到新目录**
- **所有开发工作必须在 worktree 目录中进行**
...
## 典型场景示例
1. **新功能开发**：用户需要开发新功能，自动创建 feature/ai-xxx 分支和独立 worktree，**自动切换到新目录**进行开发
2. **Bug 修复**：用户报告问题，创建 fix/ai-xxx 分支和独立 worktree，**自动切换到新目录**进行修复
...
|------|------|----------|
| 1 | `git branch` | 当前分支正确 |
| 2 | `git status` | 工作目录干净或已保存 |
| 3 | `git worktree add` | 创建 worktree 成功 |
| 4 | `cd <worktree_dir>` | **切换到 worktree 目录** |
| 5 | `pwd && git branch` | 确认在正确的目录和分支 |
...
|------|------|----------|
| 1 | `git log` | 确认合并成功 |
| 2 | `git worktree list` | 确认 worktree 存在 |
| 3 | 用户确认 | 获得用户明确同意 |
...
| 错误场景 | 原因 | 解决方案 |
|----------|------|----------|
| 创建 worktree 后仍在原目录修改 | 忘记切换目录 | 创建 worktree 后立即执行 `cd ../moe_social_<功能名>` |
| 原目录代码被修改 | 在错误的目录开发 | 始终在 worktree 目录中进行开发 |
| 合并冲突 | 原分支有未合并的修改 | 合并前确保原分支状态干净 |
| worktree 删除失败 | worktree 目录有未提交修改 | 提交或撤销修改后再删除 |