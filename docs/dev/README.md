# 开发与联调文档

> 总索引：[docs/README.md](../README.md)

## 后端（Kratos HTTP）

| 文档 | 用途 |
|------|------|
| [kratos-migration-status.md](./kratos-migration-status.md) | **状态板** |
| [kratos-migration.md](./kratos-migration.md) | 架构 SSOT |
| [new-api-kratos.md](./new-api-kratos.md) | 新接口开发 |
| [openapi-apifox.md](./openapi-apifox.md) | Apifox |
| [deploy-platform.md](./deploy-platform.md) | 云平台部署 |
| [../../backend/LAYOUT.md](../../backend/LAYOUT.md) | 目录 |

## 前端 / 联调

| 文档 | 用途 |
|------|------|
| [环境配置说明.md](./环境配置说明.md) | API 基址 |
| [快速调试步骤.md](./快速调试步骤.md) | Flutter |
| [ports.md](./ports.md) | 端口 |
| [API调试指南.md](./API调试指南.md) | 接口调试 |

## 记忆与智能栈

| 文档 | 用途 |
|------|------|
| [llm-inference-and-memory-vision.md](./llm-inference-and-memory-vision.md) | 推理 + 记忆 |
| [用户记忆系统-OpenClaw式演进设计.md](./用户记忆系统-OpenClaw式演进设计.md) | 记忆架构 |

## 已归档

go-zero / 分体 api+rpc / compat 迁移专文 → [../archive/dev/migration-legacy/](../archive/dev/migration-legacy/)
