...
| ~~P2~~ | ~~无 MMR~~ | OpenClaw search | ✅ `rerank.go` MMR + 新近度 |
| P2 | 无专用 cross-encoder rerank API | Mem0 reranker | 可选接中转 rerank 模型 |
| P2 | 图谱非 LLM 三元组 | Mem0 graph | ✅ 规则簇 + `user_memory_relations` |
...
详见 [用户记忆系统-OpenClaw式演进设计.md](./用户记忆系统-OpenClaw式演进设计.md) 变更记录节。
...
- 本项目：[用户记忆系统-OpenClaw式演进设计.md](./用户记忆系统-OpenClaw式演进设计.md)  
