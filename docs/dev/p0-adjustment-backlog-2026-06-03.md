...
- P0-1：底栏移出的同好、社区、小游戏、抽卡等能力在首页「功能入口」与我的「常用功能」中展示，避免用户误以为功能消失。
- P0-1：底栏「消息」回归私信主页面；同好、在线匹配、添加好友、申请处理通过右上角入口/空状态按钮打开面板，不替换当前私信上下文，避免后续无限加 tab。
- P0-1：2026-06-04 追加消息页 UX 收束：同好入口改为轻量「同好工作台」面板，统计、匹配、添加、申请、搜索、联系人列表在面板内完成；不再把完整同好页硬塞进消息 tab 或弹层。
- P0-1：2026-06-04 追加匹配面板 compact 样式：在线匹配、话题选择、推荐按钮降密度，作为消息页的临时任务面板，而不是替代当前私信操作。
...
| 探索页职责过重 | 探索混合匹配、AI、小游戏 | 探索不再挂底栏；AI 提升为底栏主入口；在线匹配合并到消息页「同好工作台」与匹配面板 | 主路径 1 步进入 AI、消息或在线匹配 |
| 消息页二级能力拥挤 | 私信、同好、匹配、添加、申请如果都做成 tab 会继续膨胀 | 消息页只保留私信列表；同好/匹配以轻量面板打开，面板内用快捷操作区承载新增能力 | 点击同好不替换当前消息列表；新增好友能力不继续增加 tab |
