...
> **当前状态**: 基础框架已完成（Tick 引擎 + 属性系统 + 行为决策 + WebSocket 广播 + 前端观察 UI），Bug 修复轮次已完成  
> **目标**: 从"观察型生态模拟"进化为"互动型数字宠物"体验
...
```
观察型生态模拟 ──→ 互动型数字宠物 ──→ 情感陪伴型 AI 生命
   (当前)            (阶段 1-4)          (阶段 5 及远期)
```

**当前状态**：用户只能"看"——打开世界页面，观察 6 个种子实体（小花🐰、时雨🦊、团子🐹、啾啾🐥、泡泡🐠、小眠🦌）在 32×18 生态网格中自主觅食、休息、繁衍、消亡。后端已有 `POST /api/life/action` 端点支持 `feed/pet/move` 三种操作（`engine.go:ApplyUserAction`），但前端尚未对接。

### 1.2 核心目标

- **让用户真正"养"一个实体**：通过喂食、抚摸、道具使用等操作建立日常互动习惯
- **建立情感连接**：实体有成长、有性格、有记忆，能回应用户的关怀
- **社交维度**：实体之间形成关系网络，产生社交行为，增加生态深度

### 1.3 最终形态

一个有温度、有记忆的数字生命陪伴系统：
- 用户拥有自己的宠物，它会成长、变老、形成独特性格
- 实体之间会交朋友、结伴、产生后代
- 用户可以用道具丰富宠物生活，装扮个性化外观
- 远期：实体能基于性格和状态"说话"（LLM 驱动），成为真正的情感陪伴
...
| 阶段 | 名称 | 周期 | 核心交付 | 验收标准 |
|------|------|------|----------|----------|
| 一 | 用户交互深化 | 1 周 | 前端操作按钮 + 后端冷却/日志 | 用户可在详情页和地图对实体执行 feed/pet，有冷却限制和事件反馈 |
| 二 | 实体成长与进化 | 2 周 | 年龄/成长阶段/经验值系统 | 实体随 tick 积累经验并成长，外观随阶段变化，有自然寿命 |
| 三 | 社交关系网络 | 2 周 | 关系表 + 亲密度 + 社交行为 | 实体间自动建立关系，行为决策受关系影响，前端可查看关系图谱 |
| 四 | 背包与道具系统 | 2 周 | 道具表 + 背包 + 使用机制 | 用户可通过签到/成就获取道具并给实体使用，有持续效果 |
| 五 | 个性化与高级玩法 | 3 周 | 命名/装扮/性格/成就/对话 | 实体有独特性格和名字，可装扮，有成就系统，远期支持 LLM 对话 |

---

## 3. 阶段一：用户交互深化（1 周）

**目标**：前端对接已有 `POST /api/life/action` 端点，用户可以操作实体，并获得视觉和事件反馈。
...
- **修改内容**:
  ...
  - `life_event_feed.dart` 中根据事件类型显示不同图标：`user_feed` → 🍖，`user_pet` → 🤚，`user_move` → 📍
  - 用户操作事件高亮显示（背景色加浅绿底）
...
| 周次 | 阶段 | 核心任务 | 交付物 |
|------|------|----------|--------|
| **Week 1** | 阶段一 | 前端操作按钮、地图长按菜单、冷却机制、操作反馈动画、后端增强 | 可交互的实体操作体验 |
| **Week 2** | 阶段二（上） | 数据模型扩展、成长阶段设计、经验系统、Proto 定义启动 | 后端成长系统运行 |
| **Week 3** | 阶段二（下） | 寿命机制、外观变化、前端成长进度条、成长通知 | 完整成长体验 |
| **Week 4** | 阶段三（上） | 关系表、亲密度系统、行为决策增强、WS 鉴权 | 社交系统后端 |
| **Week 5** | 阶段三（下） | 社交事件、关系图谱 UI、用户管理关系 | 可交互关系图谱 |
| **Week 6** | 阶段四（上） | 道具表、背包表、初始道具数据、获取机制 | 道具基础设施 |
| **Week 7** | 阶段四（下） | 使用机制、持续效果、前端背包 UI、商店（可选） | 完整道具使用体验 |
| **Week 8** | 阶段五（上） | 命名/改名、装扮、性格系统 | 实体个性化 |
| **Week 9** | 阶段五（中） | 成就系统、性格影响行为、实体日记 | 深度玩法 |
| **Week 10** | 阶段五（下） | LLM 对话（如有条件）、分享功能、全面测试 | 完整产品体验 |
