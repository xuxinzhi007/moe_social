...
// UserMemoryDisplayData 面向用户展示的记忆视图。
...
// UserMemoryDisplayProfile 记忆画像摘要。
...
// UserMemoryDisplayItem 单条用户可见记忆。
...
// BuildUserMemoryDisplay proto 记忆 → 展示 DTO。
func BuildUserMemoryDisplay(memories []*llmv1.UserMemory, profiles []*llmv1.UserMemoryProfile) UserMemoryDisplayData {
	...
	for _, m := range memories {
		...
		items = append(items, UserMemoryDisplayItem{
			ID: m.Id, Key: key, Title: memoryTitleFromKey(key), Content: value,
			Category: memoryCategoryLabel(mType, key), UpdatedAt: m.UpdatedAt,
		})
	}
	...
	for _, p := range profiles {
		...
		displayProfiles = append(displayProfiles, UserMemoryDisplayProfile{
			Title: profileTitleFromType(p.MemoryType), Summary: summary, ItemCount: int(p.ItemCount),
		})
	}
	...
	return UserMemoryDisplayData{
		Headline: headline, Profiles: displayProfiles, Items: items, Total: len(items),
	}
}

// BuildUserMemoryDisplayFromAPI 从 API types 构建（测试或聚合用，由 api 子树调用）。
func BuildUserMemoryDisplayFromAPI(memories []UserMemoryInput, profiles []UserMemoryProfileInput) UserMemoryDisplayData {
	rpcMem := make([]*llmv1.UserMemory, 0, len(memories))
	for _, m := range memories {
		rpcMem = append(rpcMem, &llmv1.UserMemory{
			Id: m.ID, Key: m.Key, Value: m.Value, MemoryType: m.MemoryType,
			Source: m.Source, UpdatedAt: m.UpdatedAt,
		})
	}
	rpcProf := make([]*llmv1.UserMemoryProfile, 0, len(profiles))
	for _, p := range profiles {
		rpcProf = append(rpcProf, &llmv1.UserMemoryProfile{
...
// UserMemoryInput API 记忆字段（避免 biz 依赖 api/internal/types）。
type UserMemoryInput struct {
	ID, Key, Value, MemoryType, Source, UpdatedAt string
}

// UserMemoryProfileInput API 画像字段。
type UserMemoryProfileInput struct {
	MemoryType string
	Summary    string
	ItemCount  int
}
