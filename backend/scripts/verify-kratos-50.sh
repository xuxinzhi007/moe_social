...
grep -q 'RegisterAdminCompat' internal/platform/moekratos/app.go
grep -q '/api/admin/moe/runtimes' api/moekratospilot/admin_compat.go
grep -q 'progress_percent' internal/server/moekratoshttp/register.go
...
if [ "${MOE_KRATOS_LIVE:-}" = "1" ]; then
  ...
  curl -sf "http://127.0.0.1:19032/migration" | grep -q '"progress_percent":50'
