...
const KEY_ACTIONS: Record<string, SchemaQuickAction[]> = {
  users: [
    { label: '编辑用户头像', to: '/users', hint: '在用户详情抽屉中修改 avatar URL' },
  ],
  gifts: [{ label: '礼物目录 CRUD', to: '/gifts/catalog' }],
  vip_plans: [{ label: '会员套餐', to: '/vip/plans' }],
  posts: [{ label: '动态审核', to: '/content/posts' }],
  comments: [{ label: '评论管理', to: '/content/comments' }],
  groups: [{ label: '兴趣社区', to: '/content/community' }],
  post_reports: [{ label: '举报处理', to: '/content/reports' }],
  achievement_definitions: [{ label: '成长体系 · 成就', to: '/app/growth?tab=achievements' }],
  level_configs: [{ label: '成长体系 · 等级', to: '/app/growth?tab=levels' }],
  check_in_rewards: [{ label: '成长体系 · 签到奖励', to: '/app/growth?tab=rewards' }],
