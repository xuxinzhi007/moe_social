...
class _RealtimeChatPageState extends State<RealtimeChatPage> {
  ...
  /// 语音消息前缀，用于区分文本与语音
  static const String _voicePrefix = '[VOICE]';

  bool _isImageContent(String content) =>
      content.startsWith(_imgPrefix) && content.length > _imgPrefix.length;

  bool _isVoiceContent(String content) =>
      content.startsWith(_voicePrefix) && content.length > _voicePrefix.length;

  String _getImageUrl(String content) => resolveMediaUrl(
        content.substring(_imgPrefix.length).trim(),
      );

  String _getVoiceUrl(String content) => resolveMediaUrl(
        content.substring(_voicePrefix.length).trim(),
      );
  ...
  Future<void> _startVoiceCall() async {
    ...
    try {
      // 生成唯一的通话频道名称
      final channelName = 'call_${currentUserId}_${widget.userId}_${DateTime.now().millisecondsSinceEpoch}';
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final showPeerAvatar = !isMe &&
                    (index == 0 || reversedMessages[index - 1].senderId != message.senderId);
                ...
                return Column(
                  children: [
                    ...
                    _buildMessageBubble(
                      ...
                      tightBottom: index > 0 && reversedMessages[index - 1].senderId == message.senderId,
                    ),
                  ],
                );
              },
  ...
  Widget _buildMessageBubble(
    ...
    if (_isImageContent(message.content)) {
    ...
    } else if (_isVoiceContent(message.content)) {
      bubbleChild = Row(
        children: [
          ...
          Expanded(
            child: Container(
              ...
              child: const Center(
                child: Text('语音消息', style: TextStyle(fontSize: 14)),
              ),
            ),
          ),
          const SizedBox(width: 8),
          Text('10″', style: TextStyle(color: textColor, fontSize: 12)),
        ],
      );
    } else {
      bubbleChild = Text(
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(
                      ...
                      color: _isRecording ? Colors.red : scheme.onSurfaceVariant,
                    ),
                    onPressed: () {
                      if (_isRecording) {
                        _stopVoiceRecording();
